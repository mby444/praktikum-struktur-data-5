...
void printIndentedTree(Node *node, int level = 0, const string &childLabel = "ROOT")
{
    if (node == nullptr)
        return;
    ...
    cout << childLabel << ": " << node->word << endl;

    printIndentedTree(node->left, level + 1, "L");
    printIndentedTree(node->right, level + 1, "R");
}
