...
#define DATABASE_FILE "visualized.txt"
...
void printArrayWithPointers(Book arr[], int n, int low, int high, int pos)
{
    cout << "\nArray ISBN:\n";
    for (int i = 0; i < n; i++)
    {
        cout << "[" << setw(11) << arr[i].isbn << "]";
    }
    cout << "\n";
...
int interpolationSearchISBN(const string &key, Book arr[], int n)
{
    ...
    cout << "\n[Visualisasi Pencarian ISBN: " << key << "]\n";
    ...
    while (low <= high)
    {
        ...
        {
            cout << "ISBN di luar rentang [" << arr[low].isbn << " ... " << arr[high].isbn << "], hentikan pencarian.\n";
            return -1;
        }
        ...
        if (highVal == lowVal)
        {
            if (key == arr[low].isbn)
            {
                cout << "Semua ISBN sama. Cocok dengan indeks " << low << endl;
                return low;
            }
            else
            {
                cout << "Semua ISBN sama tapi tidak cocok.\n";
                return -1;
            }
        }
        ...
        {
            cout << "Posisi interpolasi di luar batas: " << pos << ".\n";
            return -1;
        }

        printArrayWithPointers(arr, n, low, high, pos);

        cout << "Low: " << low << " (" << arr[low].isbn << ")"
             << " | High: " << high << " (" << arr[high].isbn << ")"
             << " | Pos: " << pos << " (" << arr[pos].isbn << ")\n";
        ...
        if (key == arr[pos].isbn)
        {
            cout << "Ditemukan pada indeks " << pos << "!\n";
            return pos;
        }
        else if (keyVal < posVal)
        {
            cout << "ISBN yang dicari lebih kecil dari posisi saat ini.\n";
            high = pos - 1;
        }
        else
        {
            cout << "ISBN yang dicari lebih besar dari posisi saat ini.\n";
            low = pos + 1;
        }
    }

    cout << "ISBN tidak ditemukan dalam data.\n";
    return -1;
}
